...
from hybrid_transformer import HybridTransformer, _cfg
...
__all__ = ['deit_small_HyT']

class HyT(HybridTransformer):
    def __init__(self, decay_parameter=0.996, input_size=244, *args, **kwargs):
        del kwargs['pretrained_cfg']  # delete the unexpected keyword argument
        del kwargs['pretrained_cfg_overlay']  # delete the unexpected keyword argument
    ...
    def forward(self, x, return_att=False, n_layers=12, attention_type='fused'):
        ...
        if return_att:
            ...
            attn_weights = torch.stack(attn_weights) if attn_weights[0] is not None else None  # 12 * B * H * N * N
            attn_weights = torch.mean(attn_weights, dim=2) if attn_weights is not None else None  # 12 * B * N * N
            mtatt = attn_weights[-n_layers:].mean(0)[:, 0:self.num_classes, self.num_classes:].reshape([n, c, h, w]) if attn_weights is not None else None
...
@register_model
def deit_small_HyT(pretrained=False, **kwargs):
    model = HyT(
