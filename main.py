...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--model', default='deit_small_HyT', type=str, metavar='MODEL',
    ...
    parser.add_argument('--attention-type', type=str, default='mct')


    parser.add_argument('--seed', default=0, type=int)

    parser.add_argument("--loss-weight", default=1.251, type=float)

    
    parser.add_argument('--task', type=str, required=True)  # BraTS, KiTS, LASC
    parser.add_argument("--volume-split", type=int, nargs=2, default=[100, 177])
    parser.add_argument('--inplane', default='T', type=str)  # T, M
    parser.add_argument('--hy-layer', default='CI', type=str)  # '', P, CI, IC
    parser.add_argument("--length", default=16, type=int, help='number of planes per volume')
...
def main(args):
    ...
    if args.finetune:
        ...
        if args.input_size == 448:  # interpolate position embedding if input size = 448
...
if __name__ == '__main__':
    ...
    if args.task == 'KiTS':
        args.loss_weight = 2.127
        args.volume_split = [100, 178]
    elif args.task == 'LASC':
        args.loss_weight = 0.472
        args.volume_split = [100, 111]

    elif args.task == 'lasc':  # LASC
        args.loss_weight = 0.472
        args.volume_split = [100, 111]
    elif args.task == 'kits23':  # kits
        args.loss_weight = 2.127
        args.volume_split = [100, 178]
    
    args.data_path = args.data_path.replace('task', args.task)
    
    train = (not args.eval) and (not args.gen_attention_maps)
    if train:
        args.finetune = 'https://dl.fbaipublicfiles.com/deit/deit_small_patch16_224-cd65a155.pth'
        if args.output_dir:
            model_name = 'Hy{}_{}_{}'.format(args.inplane, args.hy_layer, args.length) if args.hy_layer else 'Vi{}'.format(args.inplane)
            args.output_dir = args.output_dir.replace('task', args.task)
            args.output_dir = args.output_dir.replace('date', datetime.datetime.now().strftime('%m-%d'))
            args.output_dir = args.output_dir.replace('model_name', model_name)
            Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    else:
        args.inplane = args.resume.split('Hy')[-1].split('_')[0] if 'Hy' in args.resume else args.resume.split('Vi')[-1]
        args.hy_layer = args.resume.split('Hy{}_'.format(args.inplane))[-1].split('_')[0] if 'Hy' in args.resume else ''
        args.length = int(args.resume.split('_')[-1]) if args.hy_layer else args.length
        model_name = 'Hy{}_{}_{}'.format(args.inplane, args.hy_layer, args.length) if args.hy_layer else 'Vi{}'.format(args.inplane)
        args.attention_type = 'patchcam' if args.inplane == 'M' else args.attention_type
    
    args.batch_size = args.batch_size // args.length
